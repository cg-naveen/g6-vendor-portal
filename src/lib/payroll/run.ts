...
async function draftPayslip(params: {
  ...
  const { db, runId, employee, config, year, month } = params;
  ...
  if (existing) {
    await db.payslipLine.deleteMany({ where: { payslipId: existing.id, autoGenerated: true } });
...
export async function generatePayrollRun(year: number, month: number) {
  ...
  await prisma.$transaction(
    async (tx) => {
      // This conditional write both verifies DRAFT and holds the run row lock
      // until every payslip refresh has committed. Finalize must acquire the
      // same lock, so it cannot interleave with generation.
      ...
      for (const employee of employees) {
        await draftPayslip({ db: tx, runId: run.id, employee, config, year, month });
      }
    },
    { timeout: 30_000 }
  );
 ...
 */
type PayslipDb = Pick<Prisma.TransactionClient, "payslip">;

async function recomputePayslipWithDb(
  db: PayslipDb,
  payslipId: string,
  config: RateConfig
): Promise<void> {
  ...
  const updated = await db.payslip.updateMany({
...
export async function recomputePayslip(
  ...
  await recomputePayslipWithDb(db, payslipId, config ?? (await loadRateConfig()));
}
...
export async function finalizePayrollRun(runId: string) {
  ...
  const lockResult = await prisma.$transaction(
    async (tx) => {
      // Acquire the same conditional row lock used by all draft mutations. This
      // freezes lines/PCB while every figure is recomputed with the exact config
      // that will be stored in rateSnapshot.
      const draftGuard = await tx.payrollRun.updateMany({
      ...
      for (const payslip of payslips) {
        await recomputePayslipWithDb(tx, payslip.id, config);
      }
